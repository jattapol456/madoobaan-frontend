    ...
    .mt-10
      ...
      .grid.grid-cols-5.gap-8.mt-10
        ...
        .col-span-2(class='md:col-span-12')
          ...
          InputProfile.input(type=text,labels="กรุณากรอกเบอร์โทรศัพท์*",error="กรอกเบอร์โทรศัพท์")
          
          InputProfile.input(type=text,labels="เบอร์โทรศัพท์*",v-for="(phone, index) in phone",:key="index")

          .mt-3.space-x-1.addphone(@click="addPhone()",v-if="phone.length < 2")
            span.text-info-400 + เพิ่มเบอร์โทรศัพท์*

          .flex.items-center.mt-5.space-x-1
            input(type="checkbox")
            label ใช้เบอร์โทรศัพท์หลักเป็น Line id

        .col-span-2(class='md:col-span-12')
          InputProfile.input(type=text,labels="อีเมลล์หลัก*")
          InputProfile.input(type=text,labels="Line_Id*")
