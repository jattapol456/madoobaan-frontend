    ...
    ValidationProvider(:name="name" :rules="rules" :mode="mode" :vid="vid" v-slot="{ errors }")
      ...
      .suggestion(ref="suggestion" :class="{ 'active': active, 'left': position.includes('left'), 'right': position.includes('right') }")
        .item(v-for="(keyword, i) in take(matchedKeywords, 300)" :key="i" :class="{ 'active': selector === i }" @mousedown.prevent="select(i)")
          p.font-noto(v-html="`${keyword.subDistrict} · ${keyword.district} · ${keyword.province} · ${matchReplace(keyword.zipcode, content)}`")
...
export default defineComponent({
  ...
  computed: {
    ...
    matchedKeywords(): IAddress[] {
      ...
      return sortBy(
        uniqWith(
          [...this.keywords].filter((each) => each.zipcode?.match(regexp)),
          (a, b) => a.id === b.id
        ),
        (address) => address.id
      )
  ...
  methods: {
    onSubmit() {
      if (!this.matchedKeywords[this.selector]?.zipcode?.length) return
      this.selected = this.matchedKeywords[this.selector]
      this.content = this.selected.zipcode || ''
    ...
    select(i: number) {
      ...
      this.model = this.selected.zipcode || ''
      this.$emit('input', this.content)
      this.$emit('select', this.selected)
      this.active = false
    },
